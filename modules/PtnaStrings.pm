...
sub InitMessageStrings {
    ...
    $i++;
    $MessageList[$i]->{'message'}                = gettext( "'%s' = '%s' is set but neither '%s' nor '%s' is set: consider setting one of them as they provide additional information about stops (their names, sequence and locations)." );
    $MessageList[$i]->{'type'}                   = gettext( "Notes" );
    $MessageList[$i]->{'option'}                 = "";
    $MessageList[$i]->{'description'}            = "";
    $MessageList[$i]->{'fix'}                    = "";
    ...
    $i++;
    $MessageList[$i]->{'message'}                = gettext( "Deprecated GTFS key '%s'. For valid key names, see the" );
    $MessageList[$i]->{'type'}                   = gettext( "Errors" );
    $MessageList[$i]->{'option'}                 = "";
    $MessageList[$i]->{'description'}            = "";
    $MessageList[$i]->{'fix'}                    = "";
    ...
    $i++;
    $MessageList[$i]->{'message'}                = gettext( "Invalid GTFS feed code ('%s') in GTFS key '%s'. For valid feed codes, see the" );
    $MessageList[$i]->{'type'}                   = gettext( "Errors" );
    $MessageList[$i]->{'option'}                 = "";
    $MessageList[$i]->{'description'}            = "";
    $MessageList[$i]->{'fix'}                    = "";
    ...
    $i++;
    $MessageList[$i]->{'message'}                = gettext( "Invalid GTFS key '%s'. For valid key names, see the" );
    $MessageList[$i]->{'type'}                   = gettext( "Errors" );
    $MessageList[$i]->{'option'}                 = "";
    $MessageList[$i]->{'description'}            = "";
    $MessageList[$i]->{'fix'}                    = "";
    ...
    $i++;
    $MessageList[$i]->{'message'}                = gettext( "Invalid GTFS key '%s'. This key must not have any feed code as suffix." );
    $MessageList[$i]->{'type'}                   = gettext( "Errors" );
    $MessageList[$i]->{'option'}                 = "";
    $MessageList[$i]->{'description'}            = "";
    $MessageList[$i]->{'fix'}                    = "";
    ...
    $i++;
    $MessageList[$i]->{'message'}                = gettext( "Mixed use of GTFS keys with and without feed code as suffix. E.g.: '%s' and '%s'." );
    $MessageList[$i]->{'type'}                   = gettext( "Errors" );
    $MessageList[$i]->{'option'}                 = "";
    $MessageList[$i]->{'description'}            = "";
    $MessageList[$i]->{'fix'}                    = "";
    ...
    $i++;
    $MessageList[$i]->{'message'}                = gettext( "Orphaned GTFS key '%s' = '%s' with no other GTFS-specific information." );
    $MessageList[$i]->{'type'}                   = gettext( "Errors" );
    $MessageList[$i]->{'option'}                 = "";
    $MessageList[$i]->{'description'}            = "";
    $MessageList[$i]->{'fix'}                    = "";
    ...
    $i++;
    $MessageList[$i]->{'message'}                = gettext( "PTv2 route: 'via' is set: %d. via-part ('%s') of 'name' does not match %d. via-value = '%s'" );
    $MessageList[$i]->{'type'}                   = gettext( "Notes" );
    $MessageList[$i]->{'option'}                 = "check-name-relaxed";
    $MessageList[$i]->{'description'}            = "";
    $MessageList[$i]->{'fix'}                    = "";
    ...
    $i++;
    $MessageList[$i]->{'message'}                = gettext( "PTv2 route: empty 'role'. Consider setting 'role'='platform' or 'role'='platform_entry_only' or 'role'='platform_exit_only'" );
    $MessageList[$i]->{'type'}                   = gettext( "Errors" );
    $MessageList[$i]->{'option'}                 = "";
    $MessageList[$i]->{'description'}            = "";
    $MessageList[$i]->{'fix'}                    = "";
    ...
    $i++;
    $MessageList[$i]->{'message'}                = gettext( "PTv2 route: from-part ('%s') of 'name' does not match 'from' = '%s'" );
    $MessageList[$i]->{'type'}                   = gettext( "Notes" );
    $MessageList[$i]->{'option'}                 = "check-name-relaxed";
    $MessageList[$i]->{'description'}            = "";
    $MessageList[$i]->{'fix'}                    = "";
    ...
    $i++;
    $MessageList[$i]->{'message'}                = gettext( "PTv2 route: to-part ('%s') of 'name' does not match 'to' = '%s'" );
    $MessageList[$i]->{'type'}                   = gettext( "Notes" );
    $MessageList[$i]->{'option'}                 = "check-name-relaxed";
    $MessageList[$i]->{'description'}            = "";
    $MessageList[$i]->{'fix'}                    = "";
    ...
    $i++;
    $MessageList[$i]->{'message'}                = ngettext( "Route: restricted access (%s) at platform without 'psv'='yes', '%s'='yes', '%s'='designated', or ...", "Route: restricted access (%s) at platforms without 'psv'='yes', '%s'='yes', '%s'='designated', or ...", 1 );
    $MessageList[$i]->{'type'}                   = gettext( "Errors" );
    $MessageList[$i]->{'option'}                 = "";
    $MessageList[$i]->{'description'}            = "";
    $MessageList[$i]->{'fix'}                    = "";
    ...
    $i++;
    $MessageList[$i]->{'message'}                = ngettext( "Route: restricted access (%s) for a school bus at platform", "Route: restricted access (%s) for a school bus at platforms", 1 );
    $MessageList[$i]->{'type'}                   = gettext( "Errors" );
    $MessageList[$i]->{'option'}                 = "";
    $MessageList[$i]->{'description'}            = "";
    $MessageList[$i]->{'fix'}                    = "";
    ...
    $i++;
    $MessageList[$i]->{'message'}                = ngettext( "Route: restricted access (%s) for pedestrians at platform", "Route: restricted access (%s) for pedestrians at platforms", 1 );
    $MessageList[$i]->{'type'}                   = gettext( "Errors" );
    $MessageList[$i]->{'option'}                 = "";
    $MessageList[$i]->{'description'}            = "";
    $MessageList[$i]->{'fix'}                    = "";
...
sub InitOptionStrings {
    ...
    $i++;
    $OptionList[$i]->{'option'}                 = "exclude-if-key-value";
    $OptionList[$i]->{'default'}                = "";
    $OptionList[$i]->{'description'}            = gettext( "Exclude route_master and route relations from analysis if they have 'key' = 'value' set." ) . "\n" .
                                                  gettext( "Example" ) . ": --exclude-if-key-value='network=TransLink'";
    $OptionList[$i]->{'image'}                  = "";
    $OptionHash{$OptionList[$i]->{'option'}}    = $i;

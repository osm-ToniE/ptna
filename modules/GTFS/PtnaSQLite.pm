        ...
        if ( _AttachToGtfsSqliteDb($gtfs_feed,$release_date) ) {
            ...
            if ( $route_id ) {
                ...
                if ( $RouteIdStatus[0] eq 'valid' ) {
                    ...
                    if ( $relation_id ) {
                        $gtfs_html_tag .= sprintf( ", <a href=\"/gtfs/compare-routes.php?feed=%s&release_date=%s&route_id=%s&relation=%s\" target=\"_blank\"><img src=\"/img/compare19.png\" title=\"%s\" style=\"height: 15px;width: 15px;vertical-align: middle;\">new!</a>",
                                                uri_escape($gtfs_feed), uri_escape($release_date),
                                                uri_escape($route_id),

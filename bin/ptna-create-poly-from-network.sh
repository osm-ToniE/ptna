...
then
    ...
    then
        ...
        then
            ...
            IDS=$(curl --user-agent="PTNA script on https://ptna.openstreetmap.de" "https://overpass-api.de/api/interpreter?data=$URI_ENCODED" | grep -F "relation id=" | sed -e 's/.*relation id="//' -e 's/".*$//')

...
then
    echo $(date "+%Y-%m-%d %H:%M:%S") "Call 'osmium fileinfo' for '$SOURCE' to get replication timestamp"

    TS=$(osmium fileinfo "$SOURCE" | grep osmosis_replication_timestamp | head -1 | sed -e 's/^.*=//')

    rm -f "$TARGET-Data-filtered.osm.pbf"

    echo $(date "+%Y-%m-%d %H:%M:%S") "Call 'osmium tags-filter' for '$SOURCE' to filter with positive list"
    ...
    then
        echo $(date "+%Y-%m-%d %H:%M:%S") "Call 'osmium tags-filter' for '$SOURCE' to filter with negative list"
        ...
        echo $(date "+%Y-%m-%d %H:%M:%S") "Filtered file (prositive list) has not been created or is empty"
        exit 1
    fi
else
